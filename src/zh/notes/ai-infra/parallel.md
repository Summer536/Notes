...
## 引言
我们先回答一下简介中提出的问题（以Nvidia的Megatron为例）

假设我们现在有16张卡，它将按照如下方法做划分：

- $tp = 2$, $cp(sp) = 2$, $dp = 2$, $pp = 2$。那么有：
  $$
  \text{num\_gpu} = tp * cp(sp) * dp * pp = 2 * 2 * 2 * 2 = 16
  $$
  假设我们的一台机器内有 8 张卡，则我们需要 2 台机器。

- 我们不考虑 ep 维度（即 $ep = 1$），因为本质上它不影响 cp 维度的并行（cp 维度是对 attention 做优化，ep 可以理解成是 MLP 层的操作）。

- 在考虑如何设置并行组的**顺序是 $tp-cp(sp)-ep-dp-pp$，我们认为越靠前的并行组，通讯量越大，所以尽量安排在同一台机器内**。例如对于 tp group，它的每一个 sub-tp group 关联的 2 张卡都位于同一台机器中。

- 由于 $dp = 2$，所以我们假设有两个 micro-batch，分别是 batch0 和 batch1。由于 $cp(sp) = 2$，每个 batch 都被从 seq 维度上切成两份。
...
- 对于 gpu0 来说，**[0, 2] 组成了 cp(sp) 组**，0 和 2 上维护着相同的模型权重，但是分别维护着同一个 batch 的 $\text{seq\_chunk0}$、$\text{seq\_chunk1}$。

- 对于 gpu0 来说，**[0, 4] 组成了 dp 组**，0 和 4 上维护着相同的模型权重，但是分别维护着不同 batch 的 $\text{seq\_chunk0}$。

So 为什么通讯量大小的顺序是 $tp-cp(sp)-ep-dp-pp$？

So 什么是tp、cp(sp)、ep、dp、pp？
...
解决该问题的核心思想是：在模型并行的基础上，进一步引入数据并行的办法，即把**原先的数据再划分成若干个batch**，送入GPU进行训练。**未划分前的数据，叫mini-batch**。在mini-batch上**再划分的数据**，叫**micro-batch**。

如上图(b)(c)所示，micro-batch划分可以直观的减少很多空泡时间。
...
## 七、Expert Parallel
专家并行(EP)是针对MoE的并行方式（[点击此处详细了解MoE架构](https://summer536.github.io/Notes/zh/notes/ai-infra/MoE.html)），它将每个expert切分到不同的gpu上，然后通过AllGather和AllReduce等通信算子，将每个expert的输出结果聚合到一起。

EP并行请看文章：[图解大模型训练系列之：DeepSpeed-Megatron MoE并行训练（原理篇）](https://zhuanlan.zhihu.com/p/681154742)的第三部分MoE训练
